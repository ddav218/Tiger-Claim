...
function createRouter(init) {
  ...
  async function handleAction(request, location, submission, matches, isFogOfWar, opts = {}) {
    ...
    if (isRedirectResult(result)) {
      ...
      if (opts && opts.replace != null) {
        ...
        // double back-buttons
        let _location = normalizeRedirectLocation(result.response.headers.get("Location"), new URL(request.url), basename);
   ...
   */
  async function startRedirectNavigation(request, redirect, isNavigation, {
    submission,
    fetcherSubmission,
    preventScrollReset,
    replace
...
function createStaticHandler$1(routes, opts) {
  ...
  async function queryImpl(request, location, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch) {
    ...
    try {
      if (isMutationMethod(request.method)) {
        let _result = await submit(request, matches, routeMatch || getTargetMatch(matches, location), requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch != null);
        return _result;
      }
  ...
  async function submit(request, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest) {
    ...
    if (isErrorResult(result)) {
      ...
      let _context = await loadRouteData(loaderRequest, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null, [boundaryMatch.route.id, result]);
...
async function convertDataStrategyResultToDataResult(dataStrategyResult) {
  ...
  if (type === ResultType.error) {
    if (isDataWithResponseInit(result)) {
      ...
      result = new ErrorResponseImpl(result.init?.status || 500, undefined, result.data);
    }
    return {
      type: ResultType.error,
      error: result,
...
function useHref(to, {
  ...
  !useInRouterContext() ? invariant$2(false,
...
function useRoutesImpl(routes, locationArg, dataRouterState, future) {
  ...
  if (locationArg) {
    ...
    !(parentPathnameBase === "/" || parsedLocationArg.pathname?.startsWith(parentPathnameBase)) ? invariant$2(false, `When overriding the location using \`<Routes location>\` or \`useRoutes(routes, location)\`, ` + `the location pathname must begin with the portion of the URL pathname that was ` + `matched by all parent routes. The current pathname base is "${parentPathnameBase}" ` + `but pathname "${parsedLocationArg.pathname}" was given in the \`location\` prop.`)  : void 0;
  ...
  if (dataRouterState) {
    for (let i = 0; i < renderedMatches.length; i++) {
      ...
      if (match.route.id) {
        ...
        let needsToRunLoader = match.route.loader && !loaderData.hasOwnProperty(match.route.id) && (!_errors || _errors[match.route.id] === undefined);
...
function createServerRoutes(manifest, routeModules, future, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId$1(manifest), spaModeLazyPromise = Promise.resolve({
  Component: () => null
})) {
...
function preventInvalidServerHandlerCall(type, route, isSpaMode) {
  if (isSpaMode) {
    let _fn = type === "action" ? "serverAction()" : "serverLoader()";
    let _msg = `You cannot call ${_fn} in SPA Mode (routeId: "${route.id}")`;
    console.error(_msg);
    throw new ErrorResponseImpl(400, "Bad Request", new Error(_msg), true);
  }
...
function createClientRoutes(manifest, routeModulesCache, initialState, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId$1(manifest), needsRevalidation) {
  return (routesByParentId[parentId] || []).map(route => {
...
function Links() {
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, criticalCss ? /*#__PURE__*/React.createElement("style", {
    ...
    key,
    link
  }) => isPageLinkDescriptor(link) ? /*#__PURE__*/React.createElement(PrefetchPageLinks, Object.assign({
    key: key
  }, link)) : /*#__PURE__*/React.createElement("link", Object.assign({
    key: key
  }, link))));
}
...
function PrefetchPageLinks({
  ...
  return /*#__PURE__*/React.createElement(PrefetchPageLinksImpl, Object.assign({
    page: page,
    matches: matches
  }, dataLinkProps));
}
...
function PrefetchPageLinksImpl({
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, dataHrefs.map(href => /*#__PURE__*/React.createElement("link", Object.assign({
    key: href,
    rel: "prefetch",
    as: "fetch",
    href: href
  }, linkProps))), moduleHrefs.map(href => /*#__PURE__*/React.createElement("link", Object.assign({
    key: href,
    rel: "modulepreload",
    href: href
...
import(${JSON.stringify(manifest.entry.module)});`;
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("script", Object.assign({}, props, {
      suppressHydrationWarning: true,
      dangerouslySetInnerHTML: createHtml(contextScript),
      type: undefined
    })), /*#__PURE__*/React.createElement("script", Object.assign({}, props, {
      suppressHydrationWarning: true,
...
const Link = /*#__PURE__*/React.forwardRef(function LinkWithRef({
  ...
  React.createElement("a", Object.assign({}, rest, prefetchHandlers, {
...
const Form = /*#__PURE__*/React.forwardRef(({
  ...
  return /*#__PURE__*/React.createElement("form", Object.assign({
    ...
    "data-discover": !isAbsolute && _discover === "render" ? "true" : undefined
  }));
});
...
function ScrollRestoration({
  ...
  return /*#__PURE__*/React.createElement("script", Object.assign({}, props, {
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
...
function useScrollRestoration({
  ...
  if (typeof document !== "undefined") {
    ...
    // eslint-disable-next-line react-hooks/rules-of-hooks
    React.useLayoutEffect(() => {
      let disableScrollRestoration = router?.enableScrollRestoration(savedScrollPositions, () => window.scrollY, getKey ? (location, matches) => getScrollRestorationKey(location, matches, basename, getKey) : undefined);
...
function getDocumentHeaders(build, context) {
  ...
  return matches.reduce((parentHeaders, match, idx) => {
    ...
    if (routeModule.headers == null) {
      let _headers = new Headers(parentHeaders);
      if (includeErrorCookies) {
        prependCookies(errorHeaders, _headers);
      }
      prependCookies(actionHeaders, _headers);
      prependCookies(loaderHeaders, _headers);
      return _headers;
    }
...
async function singleFetchLoaders(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
  try {
    ...
    let loadRouteIds = new URL(request.url).searchParams.get("_routes")?.split(",") || undefined;
    let result = await staticHandler.query(handlerRequest, {
      requestContext: loadContext,
      skipLoaderErrorBubbling: true,
      dataStrategy: getSingleFetchDataStrategy({
...
const createRequestHandler = (build, mode) => {
  ...
  return async function requestHandler(request, loadContext = {}) {
    ...
    if (url.pathname.endsWith(".data")) {
      ...
      let criticalCss = mode === ServerMode.Development ? await getDevServerHooks()?.getCriticalCss?.(_build, url.pathname) : undefined;
...
async function handleDocumentRequest(serverMode, build, staticHandler, request, loadContext, handleError, criticalCss) {
  ...
  try {
    ...
    entryContext = {
      ...
      serverHandoffStream: encodeViaTurboStream(_state, request.signal, build.entry.module.streamTimeout, serverMode),
      renderMeta: {}
    };
    try {
      return await handleDocumentRequestFunction(request, context.statusCode, headers, entryContext, loadContext);
...
function createSessionStorage({
  ...
  return {
    ...
    async commitSession(session, options) {
      ...
      let expires = options?.maxAge != null ? new Date(Date.now() + options.maxAge * 1000) : options?.expires != null ? options.expires : cookie.expires;
      if (id) {
